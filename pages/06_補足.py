...
def main():
    ...
    """ 
    秘伝は大きくステータス秘伝(青)、適正秘伝(緑)、技秘伝(赤)、大会秘伝(白)の4つに分類され、さらにそれぞれ★1～★3のレアリティが付与されます。  
    ...
    モンスターに大会秘伝(白)が伝授する合計数は相性値により変化し、また相性値によって、さらに上昇量にボーナスが付与されます。(たくさんついていてもすべて発動するわけではないのです。)  
    相性値と発動数、発動時のボーナス(発動効果に対して記載の数値を乗算する)は以下の通り。  
      | 記号 | 最低発動数 | 最大発動数 | ボーナス | 
      |:--|:--:|:--:| :--:| 
      | ×  | - | - | - |
      | △ | - | - | - |
    ...
    簡単に記載すると、伝授モンスター内で共通した大会秘伝(白)がある場合にボーナス値が相性値に加算されるというものです。(具体的な記載については次回以降のアップデートで予定しています。)  
    ...
    * **レアモンの扱いについて**  
    ...
    &nbsp;
    ##### モンスターの相性表（メイン血統）
    エラーで表示されない場合は、"相性検索"ページを一度開いてください。  
